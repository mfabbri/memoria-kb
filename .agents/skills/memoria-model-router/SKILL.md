...
|---|---|---|---|---|---|
| low | discovery | scanner | gpt-5.6-luna | low | no |
| low | document review | docs_reviewer | gpt-5.6-luna | medium | no |
| low | docs/planner/agent config edit | docs_editor | gpt-5.6-luna | medium | si, solo confini indicati |
| medium | code/config runtime entro contratti esistenti | implementer | gpt-5.6-terra | medium | si |
| review | regressioni, edge case, provenance, quality gate | test_reviewer | gpt-5.6-terra | high | no |
| high | architettura, migrazione, conflitto di contratti | architect | gpt-5.6-sol | high | no |
...
4. Altrimenti, se `write_set` contiene solo docs, planner, AGENTS, `.codex` o `.agents` -> `low/docs_editor`.
